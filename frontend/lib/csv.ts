...
/**
 * Processes a CSV file by parsing it into Document objects.
 * @param file - The CSV file to process.
 * @returns An array of Document objects extracted from the CSV.
 */
export async function processCSV(file: File): Promise<Document[]> {
  const buffer = await bufferFile(file);
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'csv-'));
